<template>
  <q-card-section
    v-if="showDateSelector || showTypeSelector || showTaxesSwitch || showItemSelector"
    class="row q-col-gutter-sm items-center"
  >
    <div v-if="showItemSelector" class="col-12 col-sm-3">
      <q-select
        v-model="selectedReferent"
        :options="referents"
        :label="t('dictionary.referent')"
        dense
        outlined
        emit-value
        map-options
    ...
    <div v-if="showTypeSelector" class="col-12 col-sm-2">
      <q-select
    ...
    <div v-if="showDateSelector" class="col-12 col-sm-2">
  ...
  <div v-else-if="!selectedReferent" class="q-mx-lg q-my-lg text-grey text-center">
    {{ t('stats_page.section.charts.referent_evolution.select_referent') }}
  </div>
  <apexchart
    v-else-if="hasData"
    type="line"
...
import { computeTrendLine } from '~/helpers/linearRegression'
...
type State = { months: string[]; amounts: (number | null)[] }
...
const now = dayjs()
const selectedReferent = ref<string | null>(null)
...
const state = ref<State>({ months: [], amounts: [] })
...
const fetchData = async () => {
  if (!selectedReferent.value) return
  ...
  try {
    ...
    const params = new URLSearchParams({
      ...
      'referent': selectedReferent.value,
      'type': transactionType.value,
    })

    const data: State = await fetch(`/stats/widget/referentEvolution?${params}`).then((res) =>
      res.json()
    )
...
watch([selectedReferent, transactionType, dateFrom, dateTo, includeTaxes], fetchData)

const trendData = computed(() => computeTrendLine(state.value.amounts))

const series = computed(() => [
  {
    name: t('stats_page.section.charts.referent_evolution.average_price'),
    type: 'line',
    data: state.value.amounts,
  },
  {
    name: t('stats_page.section.charts.referent_evolution.trend'),
    type: 'line',
...
const chartOptions = computed(() => ({
  ...
  stroke: {
    width: [3, 2],
    dashArray: [0, 6],
    curve: ['smooth', 'straight'] as any,
  },
  theme: {
    mode: isDark.value ? 'dark' : 'light',
  },
  colors: [cssVariable('--q-secondary'), cssVariable('--q-negative')],
  xaxis: {
