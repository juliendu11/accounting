<template>
  ...
  <div v-else-if="!selectedCategory" class="q-mx-lg q-my-lg text-grey text-center">
    {{ t('stats_page.section.charts.category_evolution.select_category') }}
  </div>
  <apexchart
    v-else-if="hasData"
    type="line"
...
const fetchData = async () => {
  ...
  try {
    ...
    const data: State = await fetch(`/stats/widget/categoryEvolution?${params}`).then((res) =>
      res.json()
    )
...
watch([selectedCategory, transactionType, dateFrom, dateTo, includeTaxes], fetchData)

const trendData = computed(() => computeTrendLine(state.value.amounts))

const series = computed(() => [
  {
    name: t('stats_page.section.charts.category_evolution.average_price'),
    type: 'line',
    data: state.value.amounts,
  },
  {
    name: t('stats_page.section.charts.category_evolution.trend'),
    type: 'line',
...
const chartOptions = computed(() => ({
  ...
  colors: [cssVariable('--q-primary'), cssVariable('--q-negative')],
  xaxis: {
